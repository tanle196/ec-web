...
function activityFromStatus(status: OrderStatus, createdAt: Date): ActivityEntry[] {
  const fmt = (d: Date) =>
  ...
  if (status === "delivered") {
    ...
    entries.push({ icon: "check", message: "Your order has been delivered. Thank you for shopping!", date: fmt(deliveredAt) });
  }

  if (["delivered", "shipped"].includes(status)) {
    const shippedAt = new Date(created);
    shippedAt.setDate(shippedAt.getDate() + 4);
    entries.push({ icon: "user", message: "Our delivery man has picked up your order for delivery.", date: fmt(shippedAt) });
    entries.push({ icon: "map-pin", message: "Your order has reached at last mile hub.", date: fmt(new Date(shippedAt.getTime() - 18 * 3600 * 1000)) });
    entries.push({ icon: "map", message: "Your order is on the way to the last mile hub.", date: fmt(new Date(shippedAt.getTime() - 48 * 3600 * 1000)) });
  }

  if (["delivered", "shipped", "processing"].includes(status)) {
    entries.push({ icon: "circle-check", message: "Your order is successfully verified.", date: fmt(new Date(created.getTime() + 1 * 3600 * 1000)) });
  }

  entries.push({ icon: "notepad", message: "Your order has been confirmed.", date: fmt(created) });
...
function ActivityIcon({ kind }: { kind: ActivityEntry["icon"] }) {
  ...
  return (
    <div className={`flex-none p-3 rounded-[2px] border ${isGreen ? "bg-[#eaf7e9] border-[#d5f0d3]" : "bg-[#eaf6fe] border-[#d5edfd]"}`}>
      {kind === "check" && (
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={isGreen ? "#2db224" : "#2da5f3"} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
          <polyline points="20 6 9 17 4 12" />
        </svg>
      )}
      {kind === "user" && (
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#2da5f3" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
      ...
      {kind === "circle-check" && (
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#2db224" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
...
function TrackOrderDetailContent({ order }: { order: OrderResponseDto }) {
  ...
  const progressPct = currentStep === 0 ? 0 : ((currentStep - 1) / (STEPS.length - 1)) * 100;
      ...
      <div className="bg-[#f2f4f5] h-18 flex items-center">
        <div className="max-w-7xl mx-auto px-16 w-full">
          <nav className="flex items-center gap-2 text-[14px] leading-5">
            ...
            <Link href="/track-order" className="text-[#5f6c72] hover:text-[#191c1f] transition-colors no-underline">
              Track Order
            </Link>
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#5f6c72" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              <polyline points="9 18 15 12 9 6" />
            </svg>
            <span className="font-medium text-[#2da5f3]">Details</span>
          </nav>
        </div>
      </div>
        ...
        <div className="border border-[#e4e7e9] rounded-[4px] overflow-hidden">
          ...
          <div className="p-6 flex flex-col gap-6">
            ...
            <p className="text-[14px] leading-5 text-[#191c1f]">
              ...
              <span className="font-medium">{fmtDate(new Date(new Date(order.createdAt).getTime() + 7 * 24 * 3600 * 1000))}</span>
            </p>
                ...
                {STEPS.map((_, i) => {
                  ...
                  return (
                    <div
                      key={i}
                      className="absolute"
                      style={{ left: `calc(${pct * 100}% * (100% - 24px) / 100)` }}
                    >
                      <ProgressDot done={currentStep > stepNum} active={currentStep === stepNum} />
                    </div>
                  );
                })}
              </div>
              ...
              <div className="flex items-start">
                {STEPS.map((step, i) => {
                  ...
                  return (
                    ...
                    >
                      <div className={`${isActive ? "text-[#fa8232]" : "text-[#77878f]"}`}>
                        {step.icon}
                      </div>
          ...
          <div className="border-t border-[#e4e7e9] p-6 flex flex-col gap-6">
            <p className="text-[18px] font-medium leading-6 text-[#191c1f]">Order Activity</p>
  ...
  return (
    <div className="min-h-screen bg-white flex items-center justify-center">
      <div className="text-center flex flex-col gap-4">
        <p className="text-[18px] text-[#191c1f] font-medium">Order not found</p>
        <Link href="/track-order" className="text-[#2da5f3] text-[14px] no-underline hover:underline">
          ← Back to Track Order
        </Link>
      </div>
    </div>
