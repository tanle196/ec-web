      ...
      <div className="flex items-center gap-0.5">
        {[1, 2, 3, 4, 5].map((i) => (
          <svg key={i} width="16" height="16" viewBox="0 0 24 24"
            fill={rating >= i ? "#FA8232" : "none"} stroke="#FA8232" strokeWidth="1.5">
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="relative aspect-square bg-gray-50 border border-border rounded-lg overflow-hidden group">
        <ProductImage src={srcs[active]} alt={mainAlt} fill className="object-contain p-8" />
        {srcs.length > 1 && (
          <>
            <button
              onClick={() => setActive((a) => (a - 1 + srcs.length) % srcs.length)}
              aria-label="Previous"
        ...
        {srcs.map((src, i) => (
          <button
            ...
            className={`flex-none w-18 h-18 rounded border-2 p-1.5 bg-gray-50 cursor-pointer transition-all ${
              active === i ? "border-primary" : "border-border hover:border-gray-400"
            }`}
          >
            <ProductImage src={src} alt="" width={52} height={52} className="w-full h-full object-contain" />
          </button>
        ))}
      </div>
    </div>
...
function HorizontalProductCard({
  ...
  img: string; name: string; price: number; slug: string;
}) {
  return (
    <Link href={`/products/${slug}`} className="flex gap-3 items-center group no-underline py-2">
      <div className="w-16 h-16 flex-none rounded bg-gray-50 border border-border overflow-hidden p-1">
        <ProductImage src={img} alt={name} width={56} height={56} className="w-full h-full object-contain" />
      </div>
      ...
      <div className="bg-white border border-border rounded-sm flex gap-8 items-start p-6">
        ...
        <div className="flex-1 min-w-0 pt-1">
          ...
          <div className="flex items-center gap-0 text-[13px] text-gray-500 mb-4 flex-wrap divide-x divide-border">
            ...
            <span className="px-3">
              Brand: <span className="text-secondary-500 font-medium">Official</span>
            </span>
          </div>
          ...
          <div className="flex items-center gap-3 mb-5">
            ...
            {product.variants.length > 1 && (() => {
              ...
              if (maxPrice > displayPrice) {
                const pct = Math.round(((maxPrice - displayPrice) / maxPrice) * 100);
                return (
                  <>
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            >
              ...
              {addedMsg ? addedMsg : addToCart.isPending ? "Adding..." : "ADD TO CART"}
            </button>
          ...
          <div className="flex items-center gap-5 text-[13px] text-gray-500 mb-5">
            ...
            {(["M24 12C24 5.37 18.63 0 12 0S0 5.37 0 12c0 5.99 4.39 10.95 10.12 11.85V15.47H7.08v-3.48h3.04v-2.65c0-3 1.79-4.66 4.52-4.66 1.31 0 2.68.23 2.68.23v2.95h-1.51c-1.49 0-1.95.93-1.95 1.87v2.25h3.32l-.53 3.48h-2.79v8.38C19.61 22.95 24 17.99 24 12z",
              ...
              <button key={idx} className="w-7 h-7 rounded-full bg-gray-100 flex items-center justify-center hover:bg-primary hover:text-white transition-colors cursor-pointer">
          ...
          <div className="border border-border rounded p-3 flex items-center gap-3">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#2DB224" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              <polyline points="20 6 9 17 4 12" />
            </svg>
            <span className="text-[13px] font-semibold text-gray-800">100% Guarantee Safe Checkout</span>
            <div className="ml-auto flex items-center gap-1.5">
              {["Visa", "MC", "AmEx", "PayPal"].map((name) => (
                <div key={name} className="h-6 px-2 bg-gray-100 rounded text-[10px] font-bold text-gray-600 flex items-center">
                  {name}
                </div>
              ))}
            </div>
        ...
        <Tabs defaultValue="description" className="flex flex-col gap-0">
          ...
          <TabsContent value="description" className="p-8">
            <div className="grid grid-cols-3 gap-10">
              <div>
                <h3 className="text-[15px] font-semibold text-gray-900 mb-3">Description</h3>
              ...
              <div>
                <h3 className="text-[15px] font-semibold text-gray-900 mb-3">Features</h3>
                <ul className="space-y-2.5">
                  {[
                    ...
                    <li key={f} className="flex items-center gap-2 text-[14px] text-gray-600">
                      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#2DB224" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                        <polyline points="20 6 9 17 4 12" />
                      </svg>
                      {f}
                    </li>
              ...
              <div>
                <h3 className="text-[15px] font-semibold text-gray-900 mb-3">Shipping Information</h3>
              ...
              <tbody>
                {(
                  [
                    ...
                    ["Variants", product.variants.length > 0 ? `${product.variants.length} available` : "None"],
                    ["Featured", product.isFeatured ? "Yes" : "No"],
                  ] as [string, string][]
                ).map(([k, v]) => (
                  <tr key={k} className="border-b border-border">
                    <td className="py-3 pr-8 text-gray-500 w-44 font-medium">{k}</td>
          ...
          <TabsContent value="specification" className="p-8">
            {product.variants.length > 0 ? (
                ...
                <tbody>
                  {product.variants.map((vr) => (
                    <tr key={vr.id} className="border-b border-border">
                      <td className="py-3 pr-8 text-gray-500 w-48 font-medium">{vr.name}</td>
              ...
              <p className="text-gray-500 text-[14px]">No specifications available.</p>
            )}
          </TabsContent>
                    ...
                    <div key={r.id} className="py-5 border-b border-border">
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <span className="text-[14px] font-semibold text-gray-900">
                          {r.user.fullName ? String(r.user.fullName) : "Anonymous"}
                        </span>
                      </div>
                      {r.content && (
                ...
                <div className="flex justify-center gap-0.5 mb-2">
                  {[1, 2, 3, 4, 5].map((i) => (
                    <svg key={i} width="18" height="18" viewBox="0 0 24 24"
                      fill={avgRating >= i ? "#FA8232" : "none"} stroke="#FA8232" strokeWidth="1.5">
                      <polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2" />
                    </svg>
                  ))}
                </div>
                <p className="text-[13px] text-gray-500">{reviews.length} Ratings</p>
              </aside>
            </div>
          </TabsContent>
        </Tabs>
      ...
      {allRelated.length > 0 && (
        <div className="mt-5 grid grid-cols-4 gap-4">
          {sections.map(({ title, items }) => (
            <div key={title} className="bg-white border border-border rounded-sm p-4">
