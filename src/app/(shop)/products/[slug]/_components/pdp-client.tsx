      ...
      <div className="flex flex-col gap-2 flex-none">
        {srcs.map((src, i) => (
          <button
            key={i}
            onClick={() => setActive(i)}
            className="w-18 h-18 rounded-[8px] bg-muted p-1.5 cursor-pointer transition-all duration-150"
            style={{
              border: active === i ? "2px solid var(--foreground)" : "1px solid var(--border)",
            }}
          >
            <Image src={src} alt="" width={60} height={60} className="w-full h-full object-contain" />
          </button>
        ))}
      </div>
      <div className="relative flex-1 aspect-square bg-muted rounded-lg p-8 flex items-center justify-center">
        <Image src={srcs[active]} alt={mainAlt} width={400} height={400} className="w-full h-full object-contain" />
      </div>
    </div>
  );
}
...
export function PdpClient({ product }: { product: ProductResponseDto }) {
  const [selectedVariant, setSelectedVariant] = useState<ProductVariantResponseDto | null>(
  ...
  return (
    <>
      <div className="grid grid-cols-[1.1fr_1fr] gap-12">
        ...
        <div>
          ...
          <div className="flex items-center gap-3.5 mb-6">
            ...
            <a href="#reviews" className="text-[13px] text-foreground underline underline-offset-3">
          ...
          {product.variants.length > 0 && (
            <div className="mb-7">
              <div className="flex items-baseline gap-2 mb-2.5">
                ...
                {selectedVariant && (
                  <span className="text-[14px] text-foreground">{selectedVariant.name}</span>
                )}
              </div>
              <ToggleGroup
                type="single"
          ...
          <div className="flex flex-col gap-3 p-5 bg-white border border-marlo-border rounded-[12px]">
            {(
              [
                ["truck", <span key="ship"><strong className="text-foreground">Miễn phí vận chuyển</strong> · giao trong 2–3 ngày</span>],
                ["refresh", <span key="ret"><strong className="text-foreground">Đổi trả miễn phí 30 ngày</strong> · không tính phí hoàn trả</span>],
                ["shield", <span key="prot"><strong className="text-foreground">Bảo vệ người mua</strong> từ Marlo · bảo đảm 90 ngày</span>],
              ] as [string, React.ReactNode][]
            ).map(([icon, text]) => (
              <div key={icon} className="flex items-center gap-3 text-[13px] text-text-secondary">
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--foreground)" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
                  {icon === "truck" && (<><path d="M5 17H3a1 1 0 0 1-1-1V4h14v12h-3M16 8h4l3 4v4a1 1 0 0 1-1 1h-2" /><circle cx="7.5" cy="17.5" r="2.5" /><circle cx="17.5" cy="17.5" r="2.5" /></>)}
                  {icon === "refresh" && (<><polyline points="1 4 1 10 7 10" /><path d="M3.51 15a9 9 0 1 0 2.13-9.36L1 10" /></>)}
      ...
      <Tabs defaultValue="details" className="mt-16 flex flex-col gap-0">
        <TabsList variant="line" className="w-full justify-start rounded-none bg-transparent border-b border-marlo-border pb-0 gap-0 h-auto">
          {TABS.map((t) => (
            <TabsTrigger
              key={t.value}
              value={t.value}
            ...
            >
              {t.value === "reviews" ? `${t.label} · ${reviews.length}` : t.label}
            </TabsTrigger>
          ))}
        </TabsList>
        ...
        <TabsContent value="specs" className="pt-8">
          {product.variants.length > 0 ? (
                ...
                {product.variants.map((vr) => (
                  <tr key={vr.id} className="border-b border-marlo-border">
                    ...
                    <td className="py-3 text-ink font-semibold">
                      {vr.price.toLocaleString("vi-VN")}₫ · Còn {vr.stock} sản phẩm
                    </td>
                  </tr>
                ))}
              </tbody>
            ...
            <p className="text-text-secondary text-[14px]">Chưa có thông số kỹ thuật.</p>
          )}
        </TabsContent>

        <TabsContent value="reviews" className="pt-8 grid grid-cols-[2fr_1fr] gap-12">
          <div>
            {reviews.length === 0 ? (
              <p className="text-text-secondary text-[14px]">Chưa có đánh giá nào.</p>
            ) : (
              reviews.map((r) => (
                <div key={r.id} className="py-5 border-b border-marlo-border">
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <span className="text-[14px] font-semibold text-ink">
                      {r.user.fullName ? String(r.user.fullName) : "Người dùng ẩn danh"}
                    </span>
                  </div>
          ...
          <aside className="bg-white border border-marlo-border rounded-[12px] p-6 self-start">
            <div className="flex items-baseline gap-2 mb-2">
              <span className="text-[56px] font-semibold text-ink leading-none tracking-[-0.03em]">0</span>
        ...
        <TabsContent value="shipping" className="pt-8">
          <div className="text-[15px] leading-relaxed text-ink space-y-3">
            <p>Giao hàng miễn phí. Dự kiến trong <strong>2–3 ngày làm việc</strong>.</p>
            <p className="text-text-secondary">
              Thành viên Marlo Prime nhận giao trong 2 giờ tại các quận nội thành, hoàn toàn miễn phí.
            </p>
          </div>
        </TabsContent>
      </Tabs>
      ...
      {relatedProducts.length > 0 && (
        <section className="mt-20">
          <SectionHeader eyebrow="Người mua cũng xem" title="Có thể bạn thích" action="Xem thêm" href="/search" />
